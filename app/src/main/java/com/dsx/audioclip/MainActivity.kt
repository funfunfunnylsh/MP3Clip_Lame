...
import com.dsx.audioclip.util.ClipUtil
...
class MainActivity : AppCompatActivity() {
    ...
    fun clip(view: View) {
        Thread {
            val src = Environment.getExternalStorageDirectory().absolutePath + "/test.mp3"
            copyAssets("test.mp3", src)
            val out = Environment.getExternalStorageDirectory().absolutePath + "/test_clip.mp3"
            ClipUtil.clip(src, out, 15 * 1000 * 1000, 25 * 1000 * 1000)
        }.start()
    }
}