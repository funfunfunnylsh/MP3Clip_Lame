...
object ClipUtil {
    ...
    fun clip(srcMp3Path: String, outPath: String, startTimeUs: Long, endTimeUs: Long) {
...
//                buffer.get(data)
        ...
        Log.d(TAG, "pcm ->  sampleRate:$sampleRate")
        Log.d(TAG, "pcm ->  channelCount:$channelCount")

//        // pcm -> WAV
//        val outWavPath = File(outPath)
//        PcmToWavUtil(sampleRate, channelCount, 16)
//            .pcmToWav(outPcmFile.absolutePath, outWavPath.absolutePath)
//        Log.d(TAG, "pcm -> WAV done:$outPath")

        // pcm -> mp3
        LameJni().pcmTomp3(outPcmFile.absolutePath,outPath,sampleRate, channelCount,128)
        Log.d(TAG, "pcm -> mp3 done:$outPath")
        if(outPcmFile.exists())  outPcmFile.delete()
    }
