...
Java_com_dsx_audioclip_util_LameJni_getVersion(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(get_lame_version());
}
pcm2mp3 *mp3_encoder;
extern "C"
JNIEXPORT int JNICALL
Java_com_dsx_audioclip_util_LameJni_pcmTomp3(JNIEnv *env, jclass clazz, jstring pcm_path,
...
Java_com_dsx_audioclip_util_LameJni_destroy(JNIEnv *env, jclass clazz) {
    if(mp3_encoder!= NULL){
        mp3_encoder->release();
        delete mp3_encoder;
        mp3_encoder= NULL;
